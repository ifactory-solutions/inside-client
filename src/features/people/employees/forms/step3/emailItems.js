...
import { getEmailDecorator, getEmailTypeDecorator } from './step3Decorators';
...
export const EmailItems = props => {
  ...
  return emails.map(email => {
    ...
    const emailTypeDecorator = getEmailTypeDecorator(
      `email-${email.hash}.type`,
      props.fieldDecorator,
      email.type
    );

    return (
      <FormItem {...FORM_ITEM_LAYOUT} label={LABELS.EMAIL} key={email.hash}>
        <Row gutter={8}>
          <Col span={12}>{emailDecorator(entries.getEmailInput())}</Col>
          <Col span={8}>{emailTypeDecorator(entries.getEmailSelector())}</Col>
          <Col span={4}>
            {emails.length > 1 && (
              <Button type="danger" onClick={() => props.onRemove(email.hash)}>
                Remover
              </Button>
            )}
          </Col>
